...
test('Test #E2 - Las preguntas sólo deben tener un signo de puntuación', () => {
    expect(fixLetter(`    hello ,how .     are you??    `)).toBe(
        'Hello, how. Are you?'
    );
});

test('Test #F - Poner en mayúscula la palabra "Santa Claus" si aparece en la carta', () => {
    expect(
        fixLetter(` hello ,santa Claus.  SAnta CLAUS santa claus   are you?`)
    ).toBe('Hello, Santa Claus. Santa Claus Santa Claus are you?');
});

test('Test #g - Poner un punto al final de la frase si no tiene puntuación', () => {
    expect(
        fixLetter(` hello ,santa Claus.  SAnta CLAUS santa claus   are you`)
    ).toBe('Hello, Santa Claus. Santa Claus Santa Claus are you.');
});
